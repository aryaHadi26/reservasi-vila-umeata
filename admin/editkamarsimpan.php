<?php
    ...
    if(empty($No_Kamar) || empty($Jenis) || empty($Type) || empty($Tarif) || empty($deskripsi)){
        ...
        window.location.href='index.php?module=editkamar&No_Kamar=$No_Kamar';</script>";
    }else{
        if($type_file == "image/jpg" || $type_file == "image/jpeg" || $type_file == "image/png"){
            ...
            if($ukuran_file <= 1000000){
                ...
                // proses upload
                $q = mysqli_query($con, "UPDATE kamar set No_Kamar='$_POST[No_Kamar]',Jenis='$_POST[Jenis]',Type='$_POST[Type]',Tarif='$_POST[Tarif]',gambar='$nama_file',deskripsi='$_POST[deskripsi]' where No_Kamar='$_POST[No_Kamar]'");
                if($q){ //cek jika proses query berhasil atau tidak
                    // jika sukses
                    move_uploaded_file($tmp_file, $path);
                    echo "<script>alert('Data Berhasil Di dirubah !!!');
